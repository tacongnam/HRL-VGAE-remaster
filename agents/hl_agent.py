...
class HLAgent:
    ...
    def select_sfc(self,
        ...
        with torch.no_grad():
            ...
            q_vecs = self.q_net(z_global_exp, sfc_feats_t, pw_exp)
        self.q_net.train()

        q_np = q_vecs.cpu().numpy()
        q_sets = [[q_np[j]] for j in range(m)]
    ...
    def train_step(self) -> Optional[float]:
        ...
        with torch.no_grad():
            ...
            for i in range(len(batch)):
                ...
                if not nfeats:
                    next_q_vecs.append(torch.zeros(N_OBJ_HL, device=self.device))
                else:
                    ...
                    q_sets_next = [[cand_np[j]] for j in range(m_next)]
                    valid_mask = np.ones(m_next, dtype=bool)
                    best_local = select_by_hypervolume(
                        q_sets_next, valid_mask, self._ref_point)
