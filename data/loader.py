...
def parse_episode(path: str) -> Tuple[nx.Graph, List[SFCRequest], List[Dict], str]:
    ...
    for name in node_names:
        ...
        G.add_node(uid, cpu_total=cpu, cpu_free=cpu, ram_total=ram, ram_free=ram, is_function_node=is_server,
    ...
    for idx, r in enumerate(R_raw):
        ...
        requests.append(SFCRequest(sfc_id=idx, source=src, dest=dst, bandwidth=bw, vnf_sequence=vnf_seq, deadline=deadline, arrival_time=arrival))
...
def get_node_features_from_graph(G: nx.Graph, d_in: int = 16) -> np.ndarray:
    ...
    is_fn = np.array([float(G.nodes[u]['is_function_node']) for u in nodes], dtype=np.float32)
    proc_delays = np.array([G.nodes[u].get('proc_delay', 0.0) for u in nodes], dtype=np.float32)
    cost_cpus = np.array([G.nodes[u].get('cost_cpu', 0.0) for u in nodes], dtype=np.float32)
    cpu_max = cpu_totals.max() if cpu_totals.max() > 0 else 1.0
    cpu_util = np.where(cpu_totals > 0, 1.0 - cpu_frees / np.where(cpu_totals > 0, cpu_totals, 1.0), 0.0)
    ...
    feats = np.stack([cpu_util, cpu_free_norm, cpu_total_norm, is_fn, avg_bw_util, degree_norm, proc_delay_norm, cost_cpu_norm], axis=1)
    if d_in > 8:
...
def train_test_split(paths: List[str], train_ratio: float = 0.8) -> Tuple[List[str], List[str]]:
