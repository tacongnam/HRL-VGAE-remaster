...
class SFCRequest:
    ...
    def to_feature_vector(self, t: int) -> np.ndarray:
        return np.array([self.source, self.dest, self.bandwidth, self.F_k, self.deadline - t, self.urgency(t), self.total_cpu_req()], dtype=np.float32)
...
def generate_sfc_requests(cfg: Config, current_t: int, next_id_start: int, rng: random.Random = None) -> List[SFCRequest]:
    if rng is None:
        rng = random.Random()
    sc = cfg.sfc
    nc = cfg.network
    ...
    for i in range(num_arrivals):
        ...
        requests.append(SFCRequest(sfc_id=next_id_start + i, source=src, dest=dst, bandwidth=bw, vnf_sequence=vnfs, deadline=deadline, arrival_time=current_t))
    return requests
