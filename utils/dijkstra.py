...
def custom_dijkstra(G: nx.Graph, source: int, target: int, req_bw: float, omega_bw: float = 100.0, lambda_penalty: float = 1.0) -> Optional[List[int]]:
    if source == target:
        return [source]
    dist = {source: 0.0}
    prev = {source: None}
...
def path_edge_arrays(G: nx.Graph, path: List[int]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def compute_load_std(G: nx.Graph) -> float:
    utils = np.array([1.0 - e['bw_free'] / e['bw_total'] for _, _, e in G.edges(data=True)], dtype=np.float64)
    if len(utils) < 2:
        return 0.0
    return float(utils.std(ddof=1))

def compute_step_reward(G: nx.Graph, path: List[int], alpha: float, beta: float, gamma_load: float) -> float:
