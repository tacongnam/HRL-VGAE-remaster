...
class MetricsTracker:
    ...
    def flush_episode(self, ep: int, env, stats: dict) -> EpisodeMetrics:
        ...
        m = EpisodeMetrics(episode=ep, hl_reward=stats['hl_reward'], ll_reward=stats['ll_reward'], vgae_loss=stats['vgae_loss'],
            hl_loss=stats['hl_loss'], ll_loss=stats['ll_loss'], accepted=stats['accepted'], rejected=stats['rejected'],
            acceptance_ratio=stats['acceptance_ratio'], avg_hops=float(np.mean(self._hop_buf)) if self._hop_buf else 0.0,
            avg_link_util=float(np.mean(link_utils)) if link_utils else 0.0, avg_cpu_util=float(np.mean(cpu_utils)) if cpu_utils else 0.0,
            total_revenue=float(np.sum(self._revenue_buf)), total_deploy_cost=stats.get('total_deploy_cost', 0.0),
            epsilon=stats.get('epsilon_hl', 1.0), w_accept=stats.get('w_accept', 0.5), w_cost=stats.get('w_cost', 0.5))
    ...
    CSV_HEADER = ("episode,hl_reward,ll_reward,vgae_loss,hl_loss,ll_loss,accepted,rejected,acceptance_ratio,"
