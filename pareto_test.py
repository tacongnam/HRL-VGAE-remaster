...
check("equal not dom", not dominates(A, np.array([10.0, -5.0])))

# ── 2. non_dominated_indices() ──────────────────────────────
print("\n=== 2. non_dominated_indices() ===")
pts = np.array([[10.0, -5.0], [8.0, -5.0], [8.0, -7.0]])
nd = set(non_dominated_indices(pts))
check("nd = {0} (A dominates B)", nd == {0}, f"got {nd}")
check(
    "nd all-front",
    set(non_dominated_indices(np.array([[10.0, 1.0], [7.0, 5.0], [3.0, 9.0]])))
    == {0, 1, 2},
)

# ── 3. non_dominated() with duplicates ──────────────────────
print("\n=== 3. non_dominated() duplicates ===")
vecs = [np.array([1.0, 1.0]), np.array([1.0, 1.0]), np.array([2.0, 2.0])]
nd2 = non_dominated(vecs)
check(
    "dominant dup only keeps dominator",
    all(np.allclose(v, [2.0, 2.0]) for v in nd2),
    f"got {nd2}",
)
...
check(
    "HV single point 3*2=6",
    abs(compute_hypervolume(np.array([[3.0, 2.0]]), ref) - 6.0) < 1e-9,
)
check("HV below ref=0", compute_hypervolume(np.array([[-1.0, -1.0]]), ref) == 0.0)
check(
    "HV 2D front >0", compute_hypervolume(np.array([[2.0, 1.0], [1.0, 2.0]]), ref) > 0
)
check(
    "HV ignores dominated",
    abs(compute_hypervolume(np.array([[3.0, 2.0], [1.0, 1.0]]), ref) - 6.0) < 1e-9,
)
check(
    "HV 4D positive",
    compute_hypervolume(
        np.array([[1.0, 1.0, 1.0, 1.0], [2.0, 0.5, 0.5, 0.5]]),
...
pruned = prune_by_hypervolume(vecs_p, 2, np.array([0.0, 0.0]))
check("Pruned to max_size=2", len(pruned) <= 2)
check("Pruned result still non-dominated", len(non_dominated(pruned)) == len(pruned))
try:
    prune_by_hypervolume(vecs_p, 0, np.array([0.0, 0.0]))
    check("max_size=0 raises ValueError", False)
except ValueError:
    check("max_size=0 raises ValueError", True)
check("Empty input returns []", prune_by_hypervolume([], 3, np.array([0.0, 0.0])) == [])

# ── 7. select_by_hypervolume() ──────────────────────────────
print("\n=== 7. select_by_hypervolume() ===")
ref_hv = np.array([-1.0, -1.0])
q_sets = [[np.array([10.0, -0.5])], [np.array([8.0, -0.5])], [np.array([8.0, -0.7])]]
mask = np.array([True, True, True])
check("HV selects best (idx 0)", select_by_hypervolume(q_sets, mask, ref_hv) == 0)

mask2 = np.array([False, True, True])
check(
    "Mask filters invalid; picks 1", select_by_hypervolume(q_sets, mask2, ref_hv) == 1
)

check(
    "Empty Q-set handled",
    select_by_hypervolume([[], [np.array([1.0, 1.0])]], np.array([True, True]), ref_hv)
    == 1,
)
...
q_tie = [[np.array([2.0, 2.0])], [np.array([2.0, 2.0])]]
...
q_cost = [[np.array([3.0, 2.0])], [np.array([2.0, 2.0])]]  # same HV if ref=-1
hv0 = hv_score_for_action([np.array([3.0, 2.0])], ref_hv)
hv1 = hv_score_for_action([np.array([2.0, 2.0])], ref_hv)
if abs(hv0 - hv1) > 1e-9:
    ...
    check(
        "Cost tie-break: prefers higher cost_obj (idx 0)",
        chosen_cost == 0,
...
ref_4 = np.array([-1.0, -1.0, -1.0, -1.0])
target = build_target_q_set(
    r_vec, next_q_sets, gamma=0.9, max_size=5, reference_point=ref_4, done=False
)
...
target_terminal = build_target_q_set(
    r_vec, next_q_sets, gamma=0.9, max_size=5, reference_point=ref_4, done=True
)
check(
    "Terminal TargetSet = [r_vec]",
    len(target_terminal) == 1 and np.allclose(target_terminal[0], r_vec),
)

target_empty = build_target_q_set(
    r_vec, [], gamma=0.9, max_size=5, reference_point=ref_4, done=False
)
check(
    "Empty next Q-sets -> [r_vec]",
...
check("A added", arch.add(np.array([10.0, -5.0]), label="A"))
check("B added", arch.add(np.array([7.0, -2.0]), label="B"))
check("C added", arch.add(np.array([3.0, 0.0]), label="C"))
check("Size=3", len(arch) == 3)
check("D rejected (dominated)", not arch.add(np.array([8.0, -6.0]), label="D"))
check("E added + C removed", arch.add(np.array([9.0, 1.0]), label="E"))
...
from models.hl_scorer import N_OBJ_HL
...
check(
    "HL buffer reward shape=(4,)",
    isinstance(stored_rv, np.ndarray) and stored_rv.shape == (4,),
)
check(
    "HL buffer valid_action_mask stored",
...
print("\n=== 14. pareto_front() helper ===")
front = set(
    map(tuple, pareto_front([(10.0, -5.0), (8.0, -5.0), (8.0, -7.0), (9.0, -4.0)]))
)
check("(9,-4) on front", (9.0, -4.0) in front)
check("(8,-7) not on front", (8.0, -7.0) not in front)
