...
class MNVGAE(nn.Module):
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, graph_id=None, update_temporal: bool = True):
        ...
        if self.temporal_enabled and graph_id is not None:
            ...
            new_state = self.temporal_cell(z_global_static.unsqueeze(0), prev.unsqueeze(0)).squeeze(0)
            z_global = new_state
            if update_temporal:
